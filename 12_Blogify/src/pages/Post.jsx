...
export default function Post() {
    ...
    return post ? (
        <div className="py-8">
            <Container>
                <div className="w-full flex justify-center mb-4 relative border rounded-xl p-2 bg-slate-200">
                    <img
                        src={appwriteService.getFilePreview(post.featuredImage)}
                        alt={post.title}
                        className="rounded-xl"
                ...
                <div className="text-white">
                    {typeof post.content === 'string' ? parse(post.content) : <p>Invalid content</p>}
                </div>
            </Container>
        </div>
    ) : null;
